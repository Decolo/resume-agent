...
[Inline approval via Wire]
  Build approval context:
    - loop adds lightweight call summary
    - loop asks each tool for optional preview text via build_approval_context()
...
### 2.1) Approval Context Ownership

Approval context is generated by tools, not by loop-side resource inspection.
...
[for each function call]
    |
    +--> summarize call args (safe inline preview)
    |
    +--> call tool.build_approval_context(**args) when available
              |
              +--> file_write/file_edit return unified diff preview
    |
    v
[compose ApprovalRequest.description]
```

Current ownership boundary:
1. Loop (`core/llm.py`) orchestrates only; it does not read target files to compute diffs.
2. Tools (`tools/file_tool.py`) own mutation preview semantics.
