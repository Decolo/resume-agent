...
class TestTextValidation:
    """Tests for Markdown/text-specific checks."""

    @pytest.mark.asyncio
    async def test_long_lines_warning(self, validator, tmp_path):
        content = "# Name\n\n" + "x" * 250 + "\n" + " ".join(["word"] * 200)
        path = _write(tmp_path, "longlines.md", content)
        result = await validator.execute(path=str(path))
        assert any(w["check"] == "text_line_length" for w in result.data["warnings"])
