...
class TestResumeLinter:
    """Core linting tests."""

    @pytest.mark.asyncio
    async def test_good_resume_scores_high(self, linter, good_resume):
        result = await linter.execute(path=str(good_resume))
        assert result.success
        assert result.data["overall_score"] >= 70

    @pytest.mark.asyncio
    async def test_weak_resume_scores_low(self, linter, weak_resume):
        result = await linter.execute(path=str(weak_resume))
        assert result.success
        assert result.data["overall_score"] < 60

    @pytest.mark.asyncio
    async def test_file_not_found(self, linter):
        result = await linter.execute(path="nonexistent.md")
        assert not result.success
        assert "not found" in result.error.lower()

    @pytest.mark.asyncio
    async def test_empty_file(self, linter, tmp_path):
        empty = tmp_path / "empty.md"
        empty.write_text("", encoding="utf-8")
        result = await linter.execute(path=str(empty))
        assert not result.success
        assert "empty" in result.error.lower()

    @pytest.mark.asyncio
    async def test_output_contains_score_breakdown(self, linter, good_resume):
        result = await linter.execute(path=str(good_resume))
    ...
    @pytest.mark.asyncio
    async def test_data_has_sections(self, linter, good_resume):
        result = await linter.execute(path=str(good_resume))
...
class TestJobDescriptionMatching:
    ...
    @pytest.mark.asyncio
    async def test_matching_jd_boosts_score(self, linter, good_resume):
        jd = "Looking for a Senior Software Engineer with Python, AWS, and Kubernetes experience."
        result = await linter.execute(path=str(good_resume), job_description=jd)
        assert result.success
    ...
    @pytest.mark.asyncio
    async def test_mismatched_jd_lowers_score(self, linter, good_resume):
    ...
    @pytest.mark.asyncio
    async def test_no_jd_still_scores(self, linter, good_resume):
        result = await linter.execute(path=str(good_resume))
        assert result.success
...
class TestFormattingChecks:
    ...
    @pytest.mark.asyncio
    async def test_tables_penalized(self, linter, tmp_path):
        content = "# Resume\n\n| Skill | Level |\n|-------|-------|\n| Python | Expert |\n\n## Experience\nDid things.\n## Education\nWent to school.\n## Skills\n- Python"
        path = tmp_path / "table_resume.md"
        path.write_text(content, encoding="utf-8")
        result = await linter.execute(path=str(path))
        fmt_issues = result.data["sections"]["formatting"]["issues"]
        assert any("table" in i.lower() for i in fmt_issues)

    @pytest.mark.asyncio
    async def test_special_chars_penalized(self, linter, tmp_path):
...
class TestCompletenessChecks:
    ...
    @pytest.mark.asyncio
    async def test_missing_email_flagged(self, linter, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_missing_experience_flagged(self, linter, tmp_path):
...
class TestStructureChecks:
    ...
    @pytest.mark.asyncio
    async def test_short_resume_flagged(self, linter, tmp_path):
        content = "# Name\nShort resume.\n\n## Experience\nWorked.\n\n## Education\nSchool.\n\n## Skills\n- Stuff"
    ...
    @pytest.mark.asyncio
    async def test_mixed_bullets_flagged(self, linter, tmp_path):
